/***************************************************************************//**
	...
	@file		FRPG_VS_Ivy.fx
	@brief		Vertex shader for Ivy variants (wall vines/climbing foliage sway)
	@par		Reconstructed from DXBC references (FRPG_Ivy_*.vpo)

	@note		Ivy wind is 2D-only (horizontal), no vertical component:
	            wind dir comes from the normal (PIN/PIWN) as a normalized
	            tangent, or from TANGENT.xz / BINORMAL.xz (PINT/PIWNT/PINTT/PIWNTT).
	            horizontal = cos(a)*cos(3a)*cos(5a)*cos(7a)
	            a = dot(dir2D, pos.xz)*0.1 + Wind1.x*Wind0.z + seed.y*Wind1.w/1024
	            amp = seed.x*Wind1.y/1024; seed = TEXCOORD1 (int2/int4) / TEXCOORD2 (float4)
	            windOff = (-hOff.x, 0, -hOff.y)
...
float2 IvyWindDir(float4 wf)
{
#if defined(WITH_Binormal) || defined(WITH_Tangent)
    return wf.xz;
#else
    return wf.xy * float2(0.0f, 2.007871f) - wf.zw * float2(2.007871f, 0.0f);
#endif
}

float4 IvyWindFrame(WT_IN In)
{
#if defined(WITH_Binormal)
    return (In.VecBin - 0.498040f) * 2.007871f;
#elif defined(WITH_Tangent)
    return (In.VecTan - 0.498040f) * 2.007871f;
#else
    return In.VecNrm.yxzy - 0.498040f;
#endif
}

float3 IvyWindDisplace(float3 pos, float2 seed, float2 dir2D)
{
#if !defined(WITH_Binormal) && !defined(WITH_Tangent)
    dir2D = normalize(dir2D);
#endif
    float amp = seed.x * gVC_WindParam_1.y * (1.0f / 1024.0f);
    float ang = gVC_WindParam_1.x * gVC_WindParam_0.z + dot(dir2D, pos.xz) * 0.1f;
    ang += seed.y * gVC_WindParam_1.w * (1.0f / 1024.0f);
    float c0 = cos(ang);
    float3 cA = cos(ang * float3(3.0f, 5.0f, 7.0f));
    float horizontal = c0 * cA.x * cA.y * cA.z;
    float2 hOff = (amp * dir2D) * horizontal;
    float3 windOff = 0.0f;
    windOff.x = -hOff.x;
    windOff.z = -hOff.y;
    return windOff;
...
#endif
    float2 dir2D = IvyWindDir(IvyWindFrame(In));
    float4 localPos;
    localPos.xyz = IvyWindDisplace(In.VecPos.xyz, seed, dir2D) + In.VecPos.xyz;
    localPos.w   = 1.0f;
    float4 worldPos;
#ifdef WITH_Skin
...
WT_OUT_VELALP VertexMain(WT_IN In)
{
    ...
    float4 ang = gVC_WindParam_1.x * gVC_WindParam_0.zzzw + dot(dir2D, In.VecPos.xz) * 0.1f;
    ang += amp2.yyyy * (1.0f / 1024.0f);
    float2 ca = cos(ang.zw);
    float4 cA = cos(ang * float4(3.0f, 5.0f, 7.0f, 3.0f));
    float2 cB = cos(ang.w * float2(5.0f, 7.0f));
    float2 hOff = ampX * dir2D;
    float horizontal = ca.x * cA.x * cA.y * cA.z;
    float4 pos1;
    pos1.xz = In.VecPos.xz - hOff * horizontal;
    pos1.y = In.VecPos.y;
    pos1.w = 1.0f;
    float horizontal2 = ca.y * cA.w * cB.x * cB.y;
    float4 pos2;
    pos2.y = In.VecPos.y;
...
WT_OUT VertexMain(WT_IN In)
{
    ...
    float2 seed = (float2)In.WindParam;
#endif
    float4 wf = IvyWindFrame(In);
#if defined(WITH_Binormal) || defined(WITH_Tangent)
    float3 localNrm = (In.VecNrm.xyz - 0.498040f) * 2.007871f;
#else
    float3 localNrm = wf.ywzy * 2.007871f;
#endif
    float2 dir2D = IvyWindDir(wf);
#if !defined(WITH_Binormal) && !defined(WITH_Tangent)
    dir2D = normalize(dir2D);
#endif
    float amp = seed.x * gVC_WindParam_1.y * (1.0f / 1024.0f);
    float ang = gVC_WindParam_1.x * gVC_WindParam_0.z + dot(dir2D, In.VecPos.xz) * 0.1f;
    ang += seed.y * gVC_WindParam_1.w * (1.0f / 1024.0f);
    float c0 = cos(ang);
    float3 cA = cos(ang * float3(3.0f, 5.0f, 7.0f));
    float horizontal = c0 * cA.x * cA.y * cA.z;
    float2 hOff = (amp * dir2D) * horizontal;
    float4 localPos;
    localPos.xyz = float3(-hOff.x, 0.0f, -hOff.y) + In.VecPos.xyz;
    localPos.w   = 1.0f;
    float4 worldPos;
#ifdef WITH_Skin
    float wsum = In.BlendWeight.x + In.BlendWeight.y + In.BlendWeight.z + In.BlendWeight.w;
...
#ifdef WITH_Skin
    #if defined(WITH_Binormal)
    localTan = (In.VecTan - 0.498040f) * 2.007871f;
#else
    localTan = wf;
#endif
