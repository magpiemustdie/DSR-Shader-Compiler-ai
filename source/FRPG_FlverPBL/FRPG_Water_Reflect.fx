...
WATER_OUT FragmentMain_WaterReflect(WATER_IN_BASE In)
{
    ...
    float reflUV_y = -In.Pos.y * gFC_SAOParams.y + 1.0f;
    float2 reflUV = N_wave.xz * gFC_WaterReflectBand + float2(screenUV.x, reflUV_y);
    float3 envSample = gSMP_ReflectionMap.Sample(gSMP_ReflectionMapSampler, reflUV);
    ...
    linCol = linCol * (1.0f - fogFactor) + gFC_FogCol.xyz * fogFactor;
