...
#ifndef ___FRPG_Common_ForwardPBL_fxh___
#define ___FRPG_Common_ForwardPBL_fxh___
...
//--------------------------------------------------------------------------------------
// Color space — original DXBC uses pow(x, 2.2) and pow(x, 0.454545) approximation
// NOT the exact IEC 61966-2-1 sRGB transfer function
//--------------------------------------------------------------------------------------
...
// PackGBuffer — matches vanilla exactly
...
#if defined(WITH_GBuffer) && !defined(GB_NO_DENOM_CLAMP)
	// ref GB light loop D-term: max(1e-8, denom) before division — but ref is heterogeneous:
	// 224 variants have the clamp (1e-8 count 3/5), 88 have NOT (count 1, only env fade).
	// The 88 no-clamp variants get GB_NO_DENOM_CLAMP in Makefile (see AGENTS.md #7).
	denom = max(denom, 9.99999994e-009);
#endif
	float tmp = alpha / denom;
...
{
	// Schlick-GGX joint visibility approximation.
	// V = G1(NdotV) * G1(NdotL) / (4 * NdotV * NdotL)
	//   = 1 / (4 * (NdotV*(1-k)+k) * (NdotL*(1-k)+k))
	// where k = alpha / 2, alpha = Roughness^2
	// This matches the original DSR DXBC assembly (Karis 2013, Schlick 1994).
	// Returns 1/denomV * 1/denomL; the /4 is applied in microfacets_brdf.
	// Ref emits TWO div l(1): view inversion (hoisted outside the light loop)
	// and per-light inversion (e.g. Gst ref 562 + 568/603 + 654, Phn 566 + 601).
	// A single 1/(denomV*denomL) would fold into one div and undercount 1.0.
	float denomV = ndv * (1.0f - Roughness * Roughness * 0.5f) + Roughness * Roughness * 0.5f;
	float denomL = ndl * (1.0f - Roughness * Roughness * 0.5f) + Roughness * Roughness * 0.5f;
	return (1.0f / denomV) * (1.0f / denomL);
}

float3 microfacets_brdf(float3 Nn, float3 Ln, float3 Vn, float3 Ks, float Ks90, float Roughness)
{
#ifdef WITH_GBuffer
	// GB ref clamps roughness to max(0.001) in PackMaterial (before c185 remap), no FitRoughness here
#else
...
// GBuffer path falloff mapping (ref GB asm: case1=qloc, case2=1/(d+1), default=bugged linear)
float GB_lampAttenuation(float distance, float lightRadius, float OneOverFalloffEndMinusStart, uint falloffMode)
{
	switch (falloffMode)
	{
	case 1:
		return qlocAttenuation(distance, lightRadius, 2.0f);
	case 2:
		return 1.0f / (distance + 1.0f);
	default:
		return buggedLinearAttenuation(distance, lightRadius, OneOverFalloffEndMinusStart);
	}
}
...
// GBuffer path variant: same lighting structure as PointLightContribution but with the
// GB falloff mapping (case1=qloc, case2=1/(d+1), default=bugged linear).
float3 GBPointLightContribution(float3 N, float3 L, float3 V,
	float3 diffColor, float3 specColor, float specF90,
	float roughness, float3 LampColor, float LampDist,
	float OneOverFalloffEndMinusStart, float LampFalloffEnd, uint falloffMode)
{
...
{
	// UE4-style mip mapping: mip = log2(roughness) * 1.2 + mipCount - 3
	// Matches original DSR DXBC assembly and UE4 ReflectionEnvironmentShared.usf
	// (ComputeReflectionCaptureMipFromRoughness: 1.2*log2(Roughness) + CubemapMaxMip - 2)
	// NOTE: no max(0.014f) clamp here — reference logs raw roughness (max lives in FitRoughness/light loop only)
	// Written as (mipCount-1) + log2*1.2 - 2 to match ref codegen (add l(-1) -> mad l(1.2) -> add l(-2))
	return (mipCount - 1.0f) + log2(lRoughness) * 1.2f - 2.0f;
}
...
{
#ifdef WITH_GBuffer
	// ref GB (ref 141-143): EnvDifMapMulCol * (LightProbeParam.x * MagicLightParam.x) * t11
	return gFC_EnvDifMapMulCol.rgb * (gFC_LightProbeParam.x * gFC_MagicLightParam.x) * texCUBElod(gSMP_LightProbeDif, float4(dominantN, 0.0f)).rgb;
#elif defined(WITH_EnvLerp) && !defined(WITH_GhostMap) && !defined(WITH_Glow)
	// EnvLerp only in Phn refs (Gst/Sfx HemEnvLerp == HemEnv, no t13/t14)
...
#ifdef WITH_GBuffer
	// ref GB (ref 156-160): lerp(t12, t14, MagicLightParam.y) * EnvSpcMapMulCol * LightProbeParam.y
	float3 spec1 = texCUBElod(gSMP_EnvSpcMap, float4(dominantR, mipLevel)).rgb;
	float3 spec2 = texCUBElod(gSMP_EnvSpcMap2, float4(dominantR, mipLevel)).rgb;
	return gFC_EnvSpcMapMulCol.rgb * gFC_LightProbeParam.y * lerp(spec1, spec2, gFC_MagicLightParam.y);
#elif defined(WITH_EnvLerp) && !defined(WITH_GhostMap) && !defined(WITH_Glow)
	// EnvLerp only in Phn refs (Gst/Sfx HemEnvLerp == HemEnv, no t13/t14)
...
#ifdef WITH_GBuffer
	// ref GB env fade (ref 338-340): abs(N.R), NO square; both Gst and Phn clamp 1e-8
	float horiz = max(9.99999994e-009, abs(ndl));
#else
...
#ifdef USE_SH
	if (gFC_SHEnabled >= 0.5f) { // ref GB: lt cb0[187].x, l(0.5) — forward keeps != 0.0f
...
// PackMaterial — matches vanilla (gFC_MaterialOverrideParams == gFC_DebugMaterialParams, same cb0[100])
MATERIAL PackMaterial(float4 albedo, float4 pblTexData, float3 normal)
{
	MATERIAL Mtl;

	if (gFC_MaterialWorkflow.x == 0) { // metalness
		Mtl.Roughness = lerp(pblTexData.r, gFC_MaterialOverrideParams.x - 1.0f, saturate(gFC_MaterialOverrideParams.x));
		...
		Mtl.Roughness     = lerp(pblTexData.a, gFC_MaterialOverrideParams.x - 1.0f, saturate(gFC_MaterialOverrideParams.x));
...
{
	// Reference: ftou + switch on gFC_ParallaxParams.y, three independent copies of the
	// tangent view dir code (case 1: no interpolation, case 2: + interpolation, default: simple parallax)
	switch ((uint)gFC_ParallaxParams.y)
	{
	case 1:
	{
		float3 tangentViewDir = float3(dot(viewDir, normalize(B)), dot(viewDir, normalize(T)), dot(viewDir, N));
		tangentViewDir = normalize(tangentViewDir);
		float2 offset = tangentViewDir.xy * heightScale;
		float2 dx = ddx(texCoords);
		float2 dy = ddy(texCoords);

		float height0 = tex2Dgrad(gSMP_Height, texCoords, dx, dy).r;
		float2 currentTexCoords = texCoords;
		float currentLayerDepth = 0.0f;
		float currentDepthMapValue = height0;

		[loop]
		while (currentLayerDepth < currentDepthMapValue)
		{
			currentTexCoords -= offset * 0.1f;
			currentDepthMapValue = tex2Dgrad(gSMP_Height, currentTexCoords, dx, dy).r;
			currentLayerDepth += 0.1f;
		}
	...
	case 2:
	{
		...
		float currentLayerDepth = 0.0f;
		float currentDepthMapValue = height0;
		float prevDepthMapValue = height0;
		...
		float beforeDepth = prevDepthMapValue - currentLayerDepth + 0.1f;

		float weight = afterDepth / (afterDepth - beforeDepth);
		float2 finalTexCoords = (currentTexCoords + offset * 0.1f) * weight + currentTexCoords * (1.0f - weight);
	...
	default:
	{
		...
		float height0 = tex2D(gSMP_Height, texCoords).r;
		return texCoords - tangentViewDir.xy * (height0 * heightScale);
	}
	}
}

// Sfx reference emits ONE POM copy (no switch on gFC_ParallaxParams.y): plain
// interpolation variant like case 2, guarded only by the height-scale if.
float2 ParallaxOcclusionMappingSingle(float2 texCoords, float heightScale, float3 viewDir, float3 T, float3 B, float3 N)
{
	float3 tangentViewDir = float3(dot(viewDir, normalize(B)), dot(viewDir, normalize(T)), dot(viewDir, N));
	tangentViewDir = normalize(tangentViewDir);
	float2 offset = tangentViewDir.xy * heightScale;
...
#endif // ___FRPG_Common_ForwardPBL_fxh___
