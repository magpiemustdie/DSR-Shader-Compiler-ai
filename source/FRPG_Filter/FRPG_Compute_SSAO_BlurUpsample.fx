// FRPG_Compute_SSAO_BlurUpsample.fx
// Reconstructed from DSR DXBC cs_5_0 (ref 239 instr, 08/17 — full reconstruction).
// Source algorithm: Microsoft MiniEngine AoBlurAndUpsampleCS.hlsli (James Stanard).
//
// Ref cbuffer (RDEF, CB0):
//   float2 InvLowResolution;   // cb0[0].xy
//   float2 InvHighResolution;  // cb0[0].zw
//   float  NoiseFilterStrength;// cb0[1].x
//   float  StepSize;           // cb0[1].y
//   float  kBlurTolerance;     // cb0[1].z
//   float  kUpsampleTolerance; // cb0[1].w
//
// Textures: t0 = LoResDB (half-res depth), t1 = HiResDB (full-res depth),
//           t2 = LoResAO1 (interleaved AO); u0 = AoResult; s0 = LinearSampler.
// Groupshared: g0 = DepthCache (1/depth), g1 = AOCache1, g2 = AOCache2.
//
// Phase A: PrefetchData — 4 pixels/thread (gather t2 AO + t0 depth into 16x16 LDS)
// Phase B: if (GI < 39) BlurHorizontally((GI/3)*16 + (GI%3)*3)  — 13x13 -> 9x13
// Phase C: if (GI < 45) BlurVertically((GI/9)*32 + GI%9)        — 9x13 -> 9x9
// Phase D: bilateral upsample 2x2 quad per thread, weights 9/3/1/3 / (|d| + tol)
//          + NoiseFilterStrength, div l(1,1,1,1) in dp4 totals.
